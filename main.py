...
async def on_message(message):
    ...
    if message.content.startswith(pre + 'help'):
        ...
        if helpCommand == '': # if no following text show general help
            ...
            await message.channel.send("```VER 1.0```: Basic functionality added. Started spamming people to test it with me :)")
...
async def check_reminders():
    ...
    while True:
        ...
        c.execute('''
            SELECT uid, digestType, nextDigest FROM users
            WHERE nextDigest = ?
        ''', (timeCurr,))
        rows = c.fetchall()
        ...
        if len(rows) > 0:
            for i in range(len(rows)):
                ...
                if digestType != 'n': # if some kind of digest is wanted
                    ...
                    if len(events) == 0:    
                        await user.send("Hello! Today is " + timeCurr + ". You have no events " + freq[0] + ", have a good " + freq[1] + "!")
                    else:
                        await user.send("Hello! Today is " + timeCurr + ". You have the following events " + freq[0] + ":")
